...
public class LocalAlbum extends BaseActivity {
	...
	public void initAdapter() {
		...
		FolderAdapter folderAdapter = new FolderAdapter(this, LocalImageHelper
				.getInstance().getFolderMap());
		listView.setAdapter(new FolderAdapter(this, LocalImageHelper
				.getInstance().getFolderMap()));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			switch (requestCode) {
				case IntentKey.REQUEST_CODE_GETIMAGE_BYCAMERA:
					String cameraPath = LocalImageHelper.getInstance()
							.getCameraImgPath();
					if (!SPValueUtil.isEmpty(cameraPath)) {
					...
					break;
				case IntentKey.REQUEST_CODE_GETIMAGE_BYLIST:// 相册返回
					setResult(RESULT_OK);
					finish();
					break;
				default:
	...
	public class FolderAdapter extends BaseAdapter {
		...
		FolderAdapter(Context context,
			...
			options = new DisplayImageOptions.Builder()
					...
					// .setImageSize(new
					// ImageSize(((YunApp)context.getApplicationContext()).getQuarterWidth(),
					// 0))
			...
			Collections.sort(folderNames, new Comparator<String>() {
				public int compare(String arg0, String arg1) {
					Integer num1 = LocalImageHelper.getInstance()
							.getFolder(arg0).size();
					Integer num2 = LocalImageHelper.getInstance()
							.getFolder(arg1).size();
		...
		public View getView(int i, View convertView, ViewGroup viewGroup) {
			...
			if (files.size() > 0) {
				ImageLoader.getInstance().displayImage(
						files.get(0).getThumbnailUri(), viewHolder.imageView,
						options);
				// ImageLoader.getInstance().displayImage(files.get(0).getThumbnailUri(),
				// new ImageViewAware(viewHolder.imageView), options, null,
				// null, files.get(0).getOrientation());
			}
			return convertView;
		}
	...
	@OnItemClick(R2.id.local_album_list)
	public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
		Intent intent = new Intent(LocalAlbum.this, LocalAlbumDetail.class);
		intent.putExtra(IntentKey.LOCAL_FOLDER_NAME, folderNames.get(i));
		// intent.setFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT);
		startActivityForResult(intent, IntentKey.REQUEST_CODE_GETIMAGE_BYLIST);
	}
}
