 ...
 * Created by GYH on 2017/6/7.
 */

public class RoundProgressBar extends View {
    /** 加载状态--加载中*/
    public static final int PROGRESS_UPLOADING = 0X100;
    /** 加载状态--加载失败*/
    public static final int PROGRESS_FAILED = 0X300;
    /**
     * 画笔对象的引用
     */
    private Paint paint;
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        progressStatus = PROGRESS_UPLOADING;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(progressStatus == PROGRESS_FAILED)
        ...
        if(textIsDisplayable && style == STROKE){
            if(progressStatus == PROGRESS_FAILED){
                ...
                int errorTextWidth = (int) textPaint.measureText("点击");
                StaticLayout staticLayout = new StaticLayout("点击\n重试", textPaint, errorTextWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
                canvas.save();
                canvas.translate(centre - errorTextWidth / 2, centre - staticLayout.getHeight()/2);
        ...
        RectF oval = new RectF(roundWidth/4, roundWidth/4, 2*centre
                - roundWidth/4, 2*centre - roundWidth/4);  //用于定义的圆弧的形状和大小的界限
    ...
    public void progressFailed(){
        progressStatus = PROGRESS_FAILED;
        setProgress(0);
    }
